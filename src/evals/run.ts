/**
 * LangSmith Evaluation Runner for QuantMind
 * 
 * Usage:
...
// ============================================================================
// Target function - wraps QuantMind agent
// ============================================================================
...
function createEvaluationRunner(sampleSize?: number) {
  return async function* runEvaluation(): AsyncGenerator<EvalProgressEvent, void, unknown> {
    ...
    const datasetName = sampleSize 
      ? `quantmind-finance-eval-sample-${sampleSize}-${Date.now()}`
      : 'quantmind-finance-eval';
    ...
    const experimentName = `quantmind-eval-${Date.now().toString(36)}`;
