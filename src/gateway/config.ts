...
const DEFAULT_GATEWAY_PATH = join(homedir(), '.quantmind', 'gateway.json');
...
export function getGatewayConfigPath(overridePath?: string): string {
  return overridePath ?? process.env.QUANTMIND_GATEWAY_CONFIG ?? DEFAULT_GATEWAY_PATH;
}
...
export function resolveWhatsAppAccount(
  ...
  const authDir = account.authDir ?? join(homedir(), '.quantmind', 'credentials', 'whatsapp', accountId);
