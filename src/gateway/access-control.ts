...
function pairingPath(): string {
  return (
    process.env.QUANTMIND_PAIRING_PATH ??
    join(homedir(), '.quantmind', 'pairing', 'whatsapp.json')
  );
}
...
export function buildPairingReply(code: string, senderId: string): string {
  return [
    'QuantMind access request received.',
    `Sender ID: ${senderId}`,
    `Approval code: ${code}`,
    'Ask the operator to approve this code in QuantMind gateway config.',
  ].join('\n');
}
